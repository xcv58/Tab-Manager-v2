...
;(function() {
          ...
          f(...safeArgs, (...cbArgs) => {
            ...
            if (chrome.runtime.lastError)
              ...
              if (parseCB) {
                ...
                resolve(cbObj)
              } else if (!cbArgs || cbArgs.length === 0) resolve()
              else if (cbArgs.length === 1) resolve(cbArgs[0])
              else resolve(cbArgs)
            }
          })
        } catch (err) {
  ...
  function applyMap(api, apiMap) {
    ...
    for (let funcDef of apiMap) {
      let funcName
      if (typeof funcDef === 'string') funcName = funcDef
      else {
        funcName = funcDef.n
      }
  ...
  applyMaps({
    ...
    networking: [
      { n: 'config', props: ['setNetworkFilter', 'finishAuthentication'] }
    ],
    notifications: [
      'create',
      'update',
