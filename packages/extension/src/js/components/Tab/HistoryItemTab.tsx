...
import type { SearchMatchMode } from 'libs/searchMatching'

type Props = { tab: HistoryItem; matchMode?: SearchMatchMode }

const Content = observer((props: Props) => {
  const { query } = useStore().searchStore
  const { title } = props.tab
  const { matchMode = 'fuzzy' } = props
